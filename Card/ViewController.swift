...
class ViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // 背面のビュー
        let name2 = nameList[currentNum + 1]
        // ビューの背景に色をつける
    ...
    @IBAction func swipeCard(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == UIGestureRecognizer.State.ended {
            if card.center.x < 50 {
                ...
                currentNum += 1
                // 6. 遷移するかどうか判定
                if currentNum >= nameList.count {
                    // 遷移する場合の処理
                    performSegue(withIdentifier: "ToLikedList", sender: self)
                } else {
                    // 遷移しない場合
                    // 次のカードへ
                    nextCurrentNum += 1    // 次のカードが最背面に行った時のリスト番号
                    selectedCardCount = currentNum % 2    // 次のカード番号。0か1にしたいので2で割ったあまりにする。
                }
                ...
                // いいねリストに追加
                likedNameList.append(nameList[currentNum])
                // 1. 飛ばしたカードを最背面の元の場所に持ってくる
                camonCard()
                // 2. 最背面に行ったカードに、次表示する情報を入れる
                newCard()
    ...
    @IBAction func dislikeButtonTapped(_ sender: UIButton) {
        ...
        UIView.animate(withDuration: 0.5, animations: {
            ...
            self.currentNum += 1
            // 5. 遷移するかどうか判定
            if self.currentNum >= self.nameList.count {
                // 遷移する場合の処理
                self.performSegue(withIdentifier: "ToLikedList", sender: self)
            } else {
                // 遷移しない場合
                // 次のカードへ
                self.nextCurrentNum += 1    // 次のカードが最背面に行った時のリスト番号
                self.selectedCardCount = self.currentNum % 2    // 次のカード番号。0か1にしたいので2で割ったあまりにする。
            }
        })
        // ボタンを使用不可にする
    ...
    @IBAction func likeButtonTaped(_ sender: UIButton) {
        ...
        UIView.animate(withDuration: 0.5, animations: {
            ...
            // いいねリストに追加
            self.likedNameList.append(self.nameList[self.currentNum])
            // 2. 飛ばしたカードを最背面の元の場所に持ってくる
            self.camonCard()
            // 3. 最背面に行ったカードに、次表示する情報を入れる
            self.newCard()
    ...
    func newCard() {
        if nextCurrentNum < nameList.count {
            // まだ表示するリストがある場合
            let name: String = nameList[nextCurrentNum]
            // 表示するビューを管理する
